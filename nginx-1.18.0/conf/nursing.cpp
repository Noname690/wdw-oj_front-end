...
public:
    ...
    void processBodies(astra::Frame& frame)
    {
        ...
        for (auto& body : bodies)
        {
            ...
            if (body.joints()[12].world_position().z == 0) {
                char s2[] = "unkown";
                strcpy(posture, s2);
            }
            else {

                if (body.joints()[9].world_position().y - 700 > body.joints()[12].world_position().y) {
                    char s2[] = "safe";
                    strcpy(posture, s2);
                }
                else {
                ...
                if (body.joints()[4].world_position().y > body.joints()[1].world_position().y) {
                    char s2[] = "\nleft hand on";
                    strcat(posture, s2);
                }
                if (body.joints()[7].world_position().y > body.joints()[1].world_position().y) {
                    char s2[] = "\nright hand on";
                    strcat(posture, s2);
                }
                distance = sqrt(body.joints()[9].world_position().x * body.joints()[9].world_position().x + body.joints()[9].world_position().z * body.joints()[9].world_position().z);
		

		 if (body.center_of_mass().y - body.joints()[12].world_position().y < 200) {
                char s3[] = "accident";
                strcpy(posture, s3);
            }
		...
		distance = distance/1000;
            ...
            if (waist[0][0] != 0 && waist[1][0] != 0 && waist[2][0] != 0)
            {
                float depth = 0;
                for (size_t i = 0; i < 2; i++)
                {
                    depth = depth + 0.5 * sqrt((waist[i][0] - waist[i + 1][0]) * (waist[i][0] - waist[i + 1][0]) + (waist[i][1] - waist[i + 1][1]) * (waist[i][1] - waist[i + 1][1]) + (waist[i][2] - waist[i + 1][2]) * (waist[i][2] - waist[i + 1][2]));
                }
                if (depth > 25) {
                    char s2[] = "move";
                    strcpy(posture, s2);
                ...
                for (size_t i = 0; i < 2; i++)
                {
                    depth = depth + 0.5 * sqrt((waist[i][0] - waist[i + 1][0]) * (waist[i][0] - waist[i + 1][0]) + (waist[i][2] - waist[i + 1][2]) * (waist[i][2] - waist[i + 1][2]));
                }
                if (depth > 50) {
                    char s2[] = "walk";
                    strcpy(posture, s2);
