...
export default function Change_pwd() {
    ...
    <Form {...layout} className='login-form'>
        <FormItem className="form_title" {...tailLayout} style={{textAlign:"left",marginLeft:"45px"}}>change password</FormItem>
        ...
        <FormItem {...tailLayout} >
            <Button type="primary" style={{width:"200px",left:"-%50"}} className='button' onClick={()=>{
                ...
                sent()
            }}>Send the verification code to my mailbox</Button>
        </FormItem>
...
function change(password,v_code) {
    ...
    return axios.get('/api/teacher/resetPwd',{
        params:{
            sid:sid,
            v_code:v_code,
            pwd:password
...
function send(sid) {
    axios.defaults.withCredentials = true;
    return axios.get('/api/teacher/sendVerifyCode',{
        params:{
            sid
        }
    });
